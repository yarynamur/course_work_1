...
class MoviesBase:
    """
    Class for representation of abstract data structure.
    """
    ...
    def set_movies_dict(self):
        '''
        Makes a dictionary that consists of movies' titles, revenues, year of release.
        '''
        with open('sorted_dict.json') as f:
            ...
            for comp in cur_year:
                ...
                for id in cur_year[comp]:
                    ...
                    payload = "{}"
                    adress = "/3/movie/" + id + \
                        "?language=en-US&api_key=f8ddb7d056e033df36a511c18ea94e17"
                    conn.request("GET", adress, payload)
                    res = conn.getresponse()
                    movie = json.loads(res.read())
                    try:
                        if movie['revenue'] != 0:
                            self.movies_dict[comp].append(
                                movie['revenue'])
                        else:
                            self.movies_dict[comp].append(
                                movie['title'])
